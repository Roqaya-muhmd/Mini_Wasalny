...
void File::loadAllGraphs(const string& filename,unordered_map<string, unordered_map<string, vector<pair<string, int>>>>& allGraphs,unordered_map<string, unordered_map<pair<string, string>, int, pair_hash>>& edgeLists)
{
	...
	while (getline(file, line)) {
		if (line.rfind("# Graph:", 0) == 0) {
			...
			if (!currentGraph.empty()) {
				allGraphs[currentGraph] = tmpGraph;
				edgeLists[currentGraph] = tmpEdgeList;
				tmpGraph.clear();
				tmpEdgeList.clear();
			}
